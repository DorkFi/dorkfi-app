...
type ShareIndex = Record<string, BorrowShareRecord>;

let index: ShareIndex = {};
let loaded = false;

function shareDir(): string {
  return config.borrowShareStorePath;
}
...
function isExpired(record: BorrowShareRecord): boolean {
...
export async function createBorrowShare(params: {
  amount: string;
  assetSymbol: string;
  network?: string;
  imageBuffer: Buffer;
}): Promise<BorrowShareRecord> {
  await ensureLoaded();
  ...
  const record: BorrowShareRecord = {
    ...
    createdAt: now,
    expiresAt: now + config.borrowShareTtlMs,
  };
...
export async function getBorrowShare(
  id: string
): Promise<BorrowShareRecord | null> {
  await ensureLoaded();
...
export async function getBorrowShareImage(
  id: string
): Promise<{ record: BorrowShareRecord; buffer: Buffer } | null> {
